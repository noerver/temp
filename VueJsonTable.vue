<template>
  <div class="table-main">
    <div v-for="(row, index) in arrData" :key="index" class="row-data m-2 d-flex">
      <div class="key p-2 text-capitalize d-inline-block">{{ keyTitle(row) }}</div>
      ...
      <div v-else-if="checkValueType(data[row]) === 'array'">
        <div v-for="(arrRow, index2) in data[row]" :key="index2" class="d-flex">
          ...
          <div v-else>
            <VueJsonTable :data="arrRow" />
          </div>
        </div>
      </div>
      <div v-else>
        <VueJsonTable :data="data[row]" />
      </div>
    </div>
  </div>
</template>
...
<script>
import VueJsonTable from "./VueJsonTable";
